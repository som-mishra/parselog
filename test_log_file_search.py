...
class TestParseTestResults(unittest.TestCase):
    ...
    def test_get_line_datetime_nodate(self):
        # Has no date
        self.assertEqual(
            None, log_file_search.get_line_datetime("No date here"))

    def test_get_line_datetime_invaliddate(self):
        # Invalid day of month
        self.assertEqual(None, log_file_search.get_line_datetime(
            "2016-02-31 16:37:55.123 Feb. does not more than 29"))

    def test_get_line_datetime_invalidtime1(self):
        # Invalid hour for time
        self.assertEqual(None, log_file_search.get_line_datetime(
            "2016-02-28 25:37:55.123 Invalid time"))

    def test_get_line_datetime_invalidtime2(self):
        # Does not contain HH:MM:SS.SSS, missing hour/minute
        self.assertEqual(None, log_file_search.get_line_datetime(
            "2016-02-28 16:55.123 Invalid time"))

    def test_get_line_datetime_invalidtime3(self):
        self.assertEqual(
            None, log_file_search.get_line_datetime("No date or time given"))
    ...
    def test_process_file2_basic(self):
        ...
        self.assertEqual(expected, log_file_search.process_file2(
            self.content1, mydate=date1, mytime=time1, mystring=string1))
    ...
    def test_process_file2_no_string(self):
        ...
        expected = self.content1[7:]
        self.assertEqual(expected, log_file_search.process_file2(
            self.content1, mydate=date1, mytime=time1))
    ...
    def test_process_file2_random_lines1(self):
        ...
        self.assertEqual(expected, log_file_search.process_file2(
            self.content2, mydate=date1, mytime=time1))
    ...
    # Search string specified is present in both a date/time line and a line
    # with no date/time
    ...
    # Content with non-date/time lines in between, and only the hour is
    ...
    def test_process_file2_more1(self):
        ...
        self.assertEqual(expected, log_file_search.process_file2(
            self.content3, mydate=date1, mytime=time1))

    # Search string is req number with non-date/time lines in between in the
    # content, and only hour is specified
    ...
    def test_validate_inputs1(self):
        ...
        self.assertEqual(
            expected, log_file_search.validate_inputs(date1, time1))
